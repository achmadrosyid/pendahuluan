...
public class PraktikanGUI07096 {

    JFrame Praktikan = new JFrame();
    JButton back, daftarbtn;
    JTextArea area = new JTextArea();
    JLabel datadiri, daftarpraktikum;
    JComboBox pilprak = new JComboBox(PraktikumEntity07096.nama);
    int cek = allobjctrl07096.praktikan.cekDaftarprak(allobjctrl07096.praktikan.praktikanEntity().getNpm());

    public PraktikanGUI07096() {
        ...
        daftarbtn.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent ae) {
                ...
                allobjctrl07096.praktikan.daftarPraktikum(indexprak, allobjctrl07096.praktikan.getData(), false);
                area.setText(datapraktikan());
            }
        });
    ...
    String datapraktikan() {
        int cek = allobjctrl07096.praktikan.cekDaftarprak(allobjctrl07096.praktikan.praktikanEntity().getNpm());
        String cekverif;
        if (allobjctrl07096.praktikan.showDaftarprak(cek).isVerified() == false) {
            cekverif = "belum diverifikasi";
        } else {
            cekverif = "sudah diverifikasi";
        }
        String text
                = "Npm = " + allobjctrl07096.praktikan.showDaftarprak(cek).getPraktikan().getNpm() + "\n"
                + "Nama = " + allobjctrl07096.praktikan.showDaftarprak(cek).getPraktikan().getNama() + "\n"
                + "No Telp = " + allobjctrl07096.praktikan.showDaftarprak(cek).getPraktikan().getNo_telp() + "\n"
                + "Tanggal Lahir = " + new SimpleDateFormat("dd-MM-yyyy")
                .format(allobjctrl07096.praktikan.showDaftarprak(cek).getPraktikan().getTgllahir()) + "\n"
                + "verifikasi = " + cekverif + "\n"
                + "Kelas = " + allobjctrl07096.praktikan.showDaftarprak(cek).getPraktikan().getKelas() + "\n"
                + "Praktikum = " + PraktikumEntity07096.nama[allobjctrl07096.praktikan.showDaftarprak(cek).getIndexPrak()];
